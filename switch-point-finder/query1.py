...
def save_report(switches: list, phases: list, total_probes: int) -> str:
    out_dir = CONFIG["output_dir"]
    os.makedirs(out_dir, exist_ok=True)
    path = os.path.join(out_dir, "query1_switch_report.txt")

    with open(path, "w") as f:
        f.write("PLAN SWITCH REPORT — IMDB JOB Query (title + cast_info + kind_type)\n")
...
def main():
    ...
    print("\n" + "=" * 50)
    print("PLAN SWITCH FINDER — IMDB JOB Query (title + cast_info + kind_type)")
    print("=" * 50 + "\n")
    ...
    # ── Save plans & report ──────────────────────────────────────────────
    for i, ph in enumerate(phases, 1):
        save_plan(ph["plan"], f"query1_phase_{i}_plan.json")

    report_path = save_report(switches, phases, total_probes)

    print(f"Plans  → {CONFIG['output_dir']}/query1_phase_*_plan.json")
