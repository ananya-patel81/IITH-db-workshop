"""
true-switch-point-finder/query1.py
...
Usage:
    python query1.py                 # reads switch-point-finder output/ directly
"""
...
CONFIG = {
    ...
    # ── JOB Query — title + cast_info + kind_type ───────────────────────
    # Sweeps production_year on movies only (kind='movie').
    # cast_info (36M rows) joined with title (656K movies) gives a
    # cumulative row count from 22 at year=1880 to 14M+ at year=1880,
    # forcing the optimizer to switch plans across the full year range.
    "query": """
        {hint}
        EXPLAIN (FORMAT JSON)
...
def read_switch_points() -> list[dict]:
    report = os.path.join(SWITCH_OUT, "query1_switch_report.txt")
    if not os.path.exists(report):
        raise FileNotFoundError(
            f"switch_report.txt not found at {SWITCH_OUT}\n"
            "Run switch-point-finder/query1.py first."
        )
    ...
    for i, sw_year in enumerate(switch_years):
        pb = i + 1
        pa = i + 2
        path_before = os.path.join(SWITCH_OUT, f"query1_phase_{pb}_plan.json")
        path_after  = os.path.join(SWITCH_OUT, f"query1_phase_{pa}_plan.json")
...
def save_report(results: list[dict], switches: list[dict]) -> str:
    out_dir = CONFIG["output_dir"]
    os.makedirs(out_dir, exist_ok=True)
    path = os.path.join(out_dir, "query1_forced_plan_report.txt")

    with open(path, "w") as f:
        f.write("FORCED PLAN REPORT — IMDB JOB Query (title + cast_info + kind_type)\n")
...
def main():
    ...
    print("\n" + "=" * 55)
    print("FORCED PLAN CHECKER — IMDB JOB Query (title + cast_info + kind_type)")
    print("=" * 55 + "\n")
    ...
    for i, sw in enumerate(switches, 1):
        ...
        save_plan(forced_A_at_S,  f"query1_switch{i}_forced_A_at_S{S}.json")
        save_plan(forced_B_at_S1, f"query1_switch{i}_forced_B_at_S{S-1}.json")
