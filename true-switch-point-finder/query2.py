"""
true-switch-point-finder/query2.py
...
Usage:
    python query2.py                 # reads switch-point-finder output/ directly
"""
...
def read_switch_points() -> list[dict]:
    report = os.path.join(SWITCH_OUT, "query2_switch_report.txt")
    if not os.path.exists(report):
        raise FileNotFoundError(
            f"switch_report.txt not found at {SWITCH_OUT}\n"
            "Run switch-point-finder/query2.py first."
        )
    ...
    for i, sw_year in enumerate(switch_years):
        pb = i + 1
        pa = i + 2
        path_before = os.path.join(SWITCH_OUT, f"query2_phase_{pb}_plan.json")
        path_after  = os.path.join(SWITCH_OUT, f"query2_phase_{pa}_plan.json")
...
def save_report(results: list[dict], switches: list[dict]) -> str:
    out_dir = CONFIG["output_dir"]
    os.makedirs(out_dir, exist_ok=True)
    path = os.path.join(out_dir, "query2_forced_plan_report.txt")

    with open(path, "w") as f:
        f.write("FORCED PLAN REPORT — IMDB JOB Query 2\n")
...
def main():
    ...
    print("\n" + "=" * 55)
    print("FORCED PLAN CHECKER — IMDB JOB Query 2")
    print("=" * 55 + "\n")
    ...
    for i, sw in enumerate(switches, 1):
        ...
        save_plan(forced_A_at_S,  f"query2_switch{i}_forced_A_at_S{S}.json")
        save_plan(forced_B_at_S1, f"query2_switch{i}_forced_B_at_S{S-1}.json")
